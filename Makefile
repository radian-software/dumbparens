...
for_compile := dumbparens.el dumbparens-tests.el
for_checkdoc := dumbparens.el
...
.PHONY: lint
lint: compile test checkdoc longlines checkindent toc ## Run all the linters
...
checkindent: ## Ensure that indentation is correct
	@tmpdir="$$(mktemp -d)"; for file in $(for_checkindent); do \
	    ...
	    emacs -Q --batch \
	        ...
	        --eval "(load (expand-file-name \"dumbparens.el\") nil t)" \
	        --eval "(load (expand-file-name \"dumbparens-tests.el\") nil t)" \
	        --eval "(find-file \"$$file\")" \
...
toc: README.md ## Update table of contents in README
	...
	@if command -v markdown-toc >/dev/null; then \
	    ...
	    echo "  --> markdown-toc missing, skipping" >&2 ; \
	fi
